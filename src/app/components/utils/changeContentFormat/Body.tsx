export default function Body() {
  ...
  return (
    <>
      <h1>형태 변환 01</h1>
      {useImageSrcExtractor(content01).includes("http") ? (
        <img src={useImageSrcExtractor(content01)} />
      ) : (
        <p>{useImageSrcExtractor(content01)}</p>
      )}
      <br />
      <h1>형태 변환 02</h1>
      {useImageSrcExtractor(content02).includes("http") ? (
        <img src={useImageSrcExtractor(content02)} />
      ) : (
        <p>{useImageSrcExtractor(content02)}</p>
      )}
    </>
  );
}
